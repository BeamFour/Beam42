...
public class RayAberrationPlot {

    public final RayAberrationResult ray_aberration_results;

    public RayAberrationPlot(RayAberrationResult ray_aberration_results) {
        this.ray_aberration_results = ray_aberration_results;
    }

    private double auto_y_scale() {
        double yscale = 0.0;
        for (TraceFanResult fan_result : ray_aberration_results.results) {
    ...
    public String plot(TraceFanResult fan_result,double yscale) {
        ...
        for (var fan: fan_result.fans) {
            ...
            plot.add_plot_data(set, Colors.get_wavelen_color(fan.wvl),"label", PlotStyleMask.InterpolatePlot.value());
        }
        String x_label;
        String y_label;
