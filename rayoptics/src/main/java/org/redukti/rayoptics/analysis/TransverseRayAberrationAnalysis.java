...
public class TransverseRayAberrationAnalysis {
    ...
    public static TraceFanResult eval_abr_fan(OpticalModel opt_model,int fi,int xy,int num_rays,boolean append_if_none,TraceOptions trace_options) {
        var seq_model =  opt_model.seq_model;
        return seq_model.trace_fan(TransverseRayAberrationAnalysis::ray_abr,fi,xy,num_rays,append_if_none,trace_options).setFanType(RayFanType.TransverseRayFan);
    }

    public static RayAberrationResult eval(OpticalModel opt_model, int num_rays, boolean append_if_none, TraceOptions trace_options) {
        ...
        for (int fi = 0; fi < fov.fields.length; fi++) {
            for (int xy = 0; xy < 2; xy++) {
                result.add(eval_abr_fan(opt_model,fi,xy,num_rays,append_if_none,trace_options));
            }
        }
        return result;
    }
