...
public class Beam42Exporter {
    ...
    double get_refractive_index_of(SurfaceType surface_type, double wvl) {
        double index = surface_type.get_refractive_index();
        if (index == 0)
            index = 1.0;
        return index;
    }
        ...
        for (var e: prescription._surfaces) {
            if (e._is_aperture_stop || e._is_field_stop) {
                sb.append(columns[Type_col].pad("iris")).append(":")
                        .append(columns[Index_col].pad(index)).append(":")
                        .append(columns[Z_col].pad(z)).append(":")
                        .append(columns[C_col].pad(get_curvature(e))).append(":")
                        .append(columns[Dia_col].pad(get_diameter(e, scenario))).append(":")
                ...
                if (e.is_even_asphere()) {
                    sb.append(columns[S_col].pad(e.get_cc()+1.0)).append(":")
                            .append(columns[A2_col].pad(get_coeff(e,0))).append(":")
                            .append(columns[A4_col].pad(get_coeff(e,1))).append(":")
                            .append(columns[A6_col].pad(get_coeff(e,2))).append(":")
                            .append(columns[A8_col].pad(get_coeff(e,3))).append(":")
                            .append(columns[A10_col].pad(get_coeff(e,4))).append(":")
                            .append(columns[A12_col].pad(get_coeff(e,5))).append(":")
                            .append(columns[A14_col].pad(get_coeff(e,6))).append(": ");
                } else {
            ...
            index = get_refractive_index_of(e, wvln);
            sb.append(System.lineSeparator());
        }
        sb.append(columns[Type_col].pad("film")).append(":")
                        .append(columns[Index_col].pad(1.0)).append(":")
                        .append(columns[Z_col].pad(z)).append(":")
                        .append(columns[C_col].pad(0)).append(":")
                        .append(columns[Dia_col].pad(prescription._diameter_image_circle)).append(":")
                        .append(columns[S_col].pad("")).append(":")
                        .append(columns[A2_col].pad("")).append(":")
                        .append(columns[A4_col].pad("")).append(":")
                        .append(columns[A6_col].pad("")).append(":")
...
//            new ColumnDef("@", 20, MathUtils.decimal_format(10)),
    ...
    record ReferenceRay(String label, Vector3 position, Vector3 direction) {}

    static String generate_rays_table(List<ReferenceRay> rayStarts) {
        StringBuilder sb = new StringBuilder();
        ...
        for (var rayStart : rayStarts) {
            sb.append(ray_columns[X0_col].pad(rayStart.position.x())).append(":")
...
//                    .append(ray_columns[At_col].pad(wvln)).append(":")
    ...
    static void generate_rays_table(Prescription prescription, int scenario, double[] fields, String[] labels, Args arguments) throws IOException {
        ...
        for (int i = 0; i < fov.fields.length; i++) {
            ...
            var rayStarts = reference_rays(model,fld);
